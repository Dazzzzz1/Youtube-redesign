...
})({"gzpQv":[function(require,module,exports,__globalThis) {
...
module.bundle.HMR_BUNDLE_ID = "bc31145e67d724f2";
"use strict";
...
},{}],"gTHBT":[function(require,module,exports,__globalThis) {
var _storageJson = require("./storage.json");
...
function searchVideos(inputText) {
    ...
    return (0, _storageJson.videos).filter((video)=>{
        return video.title.toLowerCase().includes(inputTextLower) || video.description.toLowerCase().includes(inputTextLower) || video.owner.toLowerCase().includes(inputTextLower);
    });
}
function showResults(results) {
    ...
    if (results.length === 0) searchResults.innerHTML = '<div class="result--notfound">Nothing was found:(</div>';
    else searchResults.innerHTML = results.map((video)=>`
    ...
    searchResults.addEventListener('mouseover', (e)=>{
        ...
        if (vidsResult) playVideo(vidsResult);
    });
    searchResults.addEventListener('mouseout', (e)=>{
        const vidsResult = e.target.closest('.vids-result');
        if (vidsResult) stopVideo(vidsResult);
    });
}
...
searchInput.addEventListener('keypress', (e)=>{
    if (e.key === 'Enter') {
        ...
        if (searchText.trim().length > 1) {
            ...
            const results = searchVideos(searchText);
            showResults(results);
        } else search.style.border = '2px solid red';
    }
});
...
},{}]},["gzpQv","gTHBT"], "gTHBT", "parcelRequireef1a", {})
